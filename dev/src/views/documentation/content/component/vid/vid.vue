<template>
    <div classs="doc-vid">
        ...
        <apiTable :items="evt" title="事 件" style="margin-top: 20px"/>
    </div>
</template>

<script>
import vid from '@/assets/vid/sample.mp4'
export default {
    name: "doc-vid",
    data(){
        return {
            ...
            evt:[
                { name:'@load', args:"(duration: Number)", description:'资源加载完成事件。参数: 资源的时长' },
                { name:'@playing', args:"(currentTime: Number)", description:'播放进度事件，每秒触发一次。参数: 当前播放时间(s)' },
                { name:'@play', args:"Undefined", description:'手动/自动开始播放事件，不包含因加载而停顿后的播放' },
                { name:'@pause', args:"Undefined", description:'手动暂停后事件' },
                { name:'@wait', args:"Undefined", description:'播放过程中，因资源加载而暂停的事件' },
                { name:'@end', args:'Undefined', description:'播放结束后事件' },
            ],
            api:[
